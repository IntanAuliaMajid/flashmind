...
<main class="w-full px-6 py-5 mt-20 max-w-2xl mx-auto">
    <h1 class="text-2xl font-bold mb-2">Tambah Kata Baru</h1>
    <p class="text-gray-600 mb-6">Kelompok: <strong><?= htmlspecialchars($kelompok['nama_kelompok_kata']) ?></strong></p>

    <form action="index.php?action=kata&method=tambah&id=<?= $idKelompok ?>" method="post" class="bg-white p-6 rounded shadow">
        <div class="mb-4">
            <label class="block mb-2 font-medium">Kata</label>
            <input type="text" name="kata" class="w-full border rounded px-3 py-2" placeholder="Masukkan kata" required>
        </div>

        <div class="mb-4">
            <label class="block mb-2 font-medium">Arti / Penjelasan</label>
            <textarea name="arti" class="w-full border rounded px-3 py-2 h-24" placeholder="Masukkan arti atau penjelasan kata" required></textarea>
        </div>

        <div class="mb-4">
            <label class="block mb-2 font-medium">Contoh (opsional)</label>
            <textarea name="contoh" class="w-full border rounded px-3 py-2 h-20" placeholder="Contoh penggunaan kata (opsional)"></textarea>
        </div>
